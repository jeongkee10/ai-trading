"""
AI Trading System - Transformer 시계열 모델
Attention 메커니즘으로 장기 의존성 포착
"""
...
class TransformerStockModel:
    ...
    def __init__(self, horizon: int = 1):
        ...
        self.mean = None
        self.std = None
        self.model_path = os.path.join(MODEL_DIR, f"transformer_h{horizon}.keras")
        self.scaler_path = os.path.join(MODEL_DIR, f"transformer_scaler_h{horizon}.pkl")
    ...
    def _build_model(self, n_features: int, n_classes: int = 3):
        try:
            ...
            # Positional encoding (간단 학습형)
            pos_embed = Dense(n_features)(inputs)
            x = Add()([inputs, pos_embed])

            # Transformer Block 1
            attn_out = MultiHeadAttention(
                num_heads=4, key_dim=n_features // 4, dropout=0.1
            )(x, x)
            ...
            attn_out2 = Dropout(0.1)(attn_out2)
            x = Add()([x, attn_out2])
            x = LayerNormalization(epsilon=1e-6)(x)

            ff2 = Dense(64, activation="relu")(x)
            ff2 = Dropout(0.1)(ff2)
            ff2 = Dense(n_features)(ff2)
            x = Add()([x, ff2])
            x = LayerNormalization(epsilon=1e-6)(x)
            ...
            logger.warning("TensorFlow 미설치 - Transformer 모델 비활성화")
            return None
    ...
    def train(self, df: pd.DataFrame) -> dict:
        ...
        if len(df_clean) < self.lookback + 50:
            logger.warning(f"Transformer 학습 데이터 부족: {len(df_clean)}건")
            return {}
        ...
        try:
            ...
            if overfit_gap > 0.15:
                logger.warning(f"Transformer H{self.horizon} 과적합 의심 | Train:{train_acc:.4f} Val:{val_acc:.4f}")

            logger.info(f"Transformer H{self.horizon} 학습 완료 | Val Acc: {val_acc:.4f} | Train Acc: {train_acc:.4f} | Gap: {overfit_gap:.4f}")
            return {
            ...
            logger.error(f"Transformer 학습 오류: {e}")
            return {}
    ...
    def _save(self):
        ...
        try:
            ...
            logger.error(f"Transformer 저장 오류: {e}")
    ...
    def _load(self):
        ...
        try:
            ...
            logger.warning(f"Transformer 로드 실패: {e}")
            self.model = None
