...
class LSTMStockModel:
    ...
    def __init__(self, horizon: int = 1):
        ...
        self.mean = None
        self.std  = None
        self.model_path = os.path.join(MODEL_DIR, f"lstm_h{horizon}.keras")
        self.scaler_path = os.path.join(MODEL_DIR, f"lstm_scaler_h{horizon}.pkl")
    ...
    def _build_model(self, n_features: int, n_classes: int = 3):
        try:
            ...
            logger.warning("TensorFlow 미설치 — LSTM 모델 비활성화")
            return None
    ...
    def train(self, df: pd.DataFrame) -> dict:
        ...
        if len(df_clean) < self.lookback + 50:
            logger.warning(f"LSTM 학습 데이터 부족: {len(df_clean)}건")
            return {}
        ...
        try:
            ...
            # Self-consistency 검증
            if overfit_gap > 0.15:
                logger.warning(f"LSTM H{self.horizon} 과적합 의심 | Train:{train_acc:.4f} Val:{val_acc:.4f} Gap:{overfit_gap:.4f}")
            if val_acc < 0.35:
                logger.warning(f"LSTM H{self.horizon} 미학습 의심 | Val Acc:{val_acc:.4f}")

            logger.info(f"LSTM H{self.horizon} 학습 완료 | Val Acc: {val_acc:.4f} | Train Acc: {train_acc:.4f} | Gap: {overfit_gap:.4f}")
            return {
